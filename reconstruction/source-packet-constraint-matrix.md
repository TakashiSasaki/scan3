...
|----|-------------|--------|---------------------|-----------------|-------------------|
| C-TOP-1 | Top-level is a closed object | implemented | schema | invalid-unknown-top-level-property | - |
| C-TOP-2 | Source descriptor is a closed object | implemented | schema | invalid-unknown-source-property | - |
| C-TOP-3 | Destination descriptor is a closed object | implemented | schema | invalid-unknown-destination-property | - |
| C-TOP-4 | File entry is a closed object | implemented | schema | invalid-unknown-file-property | - |
...
| C-PATH-RAW | Direct JS-level raw relative path validation defense-in-depth | implemented | operational | - | source-packet-path-helper |
...
| C-REQ-1 | Required fields at top level | implemented | schema | invalid-missing-top-level-field, invalid-missing-format-version, invalid-missing-purpose, invalid-missing-source, invalid-missing-destination, invalid-missing-files, invalid-missing-owner-decisions | - |
| C-REQ-FILE-OBJ | File entry must be an object | implemented | schema | invalid-file-entry-type | - |
| C-REQ-OWNER-OBJ | Owner decision must be an object | implemented | schema | invalid-owner-decision-array | - |
| C-TYPE-STR-TYPE | Non-blank string type check | implemented | schema | invalid-packet-id-type, invalid-purpose-type, invalid-source-repository-type | - |
| C-TYPE-NOTES-TYPE | Notes property string type check | implemented | schema | invalid-notes-type | - |
| C-TYPE-STR-PATTERN | Non-blank string pattern check | implemented | schema | invalid-empty-string, invalid-blank-string | - |
| C-TYPE-SHA1 | Commit SHA type | implemented | schema | invalid-commit-sha | - |
| C-TYPE-SHA256 | SHA-256 type | implemented | schema | invalid-sha256-format | - |
| C-TYPE-INT-TYPE | Integer type check for sizeBytes | implemented | schema | invalid-noninteger-size | - |
| C-TYPE-INT-MIN | Minimum value check for sizeBytes | implemented | schema | invalid-negative-size | - |
| C-TYPE-INT-MAX | Maximum value check for sizeBytes | implemented | schema | invalid-large-size | - |
| C-ENUM-DISP | Disposition enum | implemented | schema | invalid-disposition | - |
| C-REQ-OWNER | Owner decision missing value | implemented | schema | invalid-missing-owner-value | - |
